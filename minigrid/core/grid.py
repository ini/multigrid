...
from minigrid.core.world_object import MyWorldObj, Empty



def MyWall(color: str = 'grey'):
    return np.array([
        OBJECT_TO_IDX['wall'], COLOR_TO_IDX[color], 0,
...
class MyGrid:
    ...
    def __init__(self, width: int, height: int):
        ...
        self.grid: np.array[int] = np.zeros((width, height, MyWorldObj.dim), dtype=int)
        self.grid[...] = Empty()
    ...
    def horz_wall(
        ...
        obj_type: Callable[[], WorldObj] = MyWall,
    ):
        if length is None:
            length = self.width - x
    ...
    def rotate_left(self, k: int) -> Grid:
        """
        Rotate the grid to the left (counter-clockwise)
        """
        new_grid_array = np.rot90(self.grid, k=k)
        new_grid = MyGrid(*new_grid_array.shape)
